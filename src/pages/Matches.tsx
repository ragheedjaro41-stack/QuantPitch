...
import { useMatches, useRealLeagues, getLeagueStatus } from "../lib/hooks";
...
export default function Matches() {
  ...
  const { data, isLoading, isError } = useMatches(leagueId);
  ...
  const subtitle = selectedLeague
    ? `${data?.length ?? 0} ${selectedLeague.name} matches`
    : `${data?.length ?? 0} matches across all leagues`;
  ...
  return (
    <div className="px-4 sm:px-8 py-6 sm:py-8 max-w-7xl mx-auto">
      <PageHeader title="Matches" subtitle={subtitle}>
        {leagues && (
          <div className="flex items-center gap-3">
            {selectedLeague && <LeagueStatusBadge status={getLeagueStatus(selectedLeague)} />}
            <LeagueSelector leagues={leagues} selected={leagueId} onChange={(id) => { setLeagueId(id); setSelectedRound(null); }} />
          </div>
        )}
      </PageHeader>
      ...
      {isLoading ? (
        ...
        <EmptyState message="No matches found for this league" />
      ) : (
        <>
          ...
          {filtered.length === 0 ? (
            ...
            <div className="space-y-3">
              {filtered.map((m) => {
                const leagueBadge = !leagueId ? leagues?.find((l) => l.id === m.league_id)?.short_name : null;
                return (
                  <Link
                    key={m.id}
                    to={`/matches/${m.id}`}
